    ...
    <div class="table-box">
      <el-table element-loading-text="数据加载中..." v-loading="loading" :data="tableData"
        ...
        <el-table-column label="预订房间">
          <template #default="scope">
            <el-tag>{{scope.row.room.roomNumber}}：{{scope.row.room.roomName}}</el-tag>
          </template>
        </el-table-column>

        <el-table-column label="总金额">
          <template #default="scope">
            <el-tag effect="plain" type="success" >￥{{scope.row.amountMoney}}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="200">
          <template #default="scope">
            ...
            <el-button size="small" type="info" color="#083a6d" @click="payment(scope.row.id)">收款</el-button>
            </template>
          </template>
        </el-table-column>
